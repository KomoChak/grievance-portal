...
export default function AreaAdminView() {
  ...
  return (
    <Paper elevation={5} sx={{ p: { xs: 2, md: 4 }, borderRadius: 4, maxWidth: 700, mx: "auto" }}>
      ...
      <Stack spacing={1} mb={2}>
        <Typography><b>Grievance No:</b> {grievanceDetails.grievanceNo}</Typography>
        <Typography><b>Subject:</b> {grievanceDetails.subject}</Typography>
        <Typography><b>Employee:</b> {grievanceDetails.employee}</Typography>
        <Typography><b>Department:</b> {grievanceDetails.department}</Typography>
        <Typography><b>Date:</b> {grievanceDetails.date}</Typography>
        <Typography><b>Description:</b> {grievanceDetails.description}</Typography>
      </Stack>
      <Divider sx={{ my: 2 }} />
      ...
      <Stack direction="row" spacing={2} alignItems="center" mb={2}>
        <Chip
          label={grievanceDetails.status}
          color={grievanceDetails.status === "Pending" ? "warning" : "success"}
          icon={<CheckCircleIcon />}
        />
        <Typography variant="body2" color="text.secondary">
          Last updated: {grievanceDetails.history[grievanceDetails.history.length - 1].date}
        </Typography>
      </Stack>
      <Box>
        ...
        {grievanceDetails.history.map((h, idx) => (
